...
public class svm_toy extends Applet {
	...
	void button_run_clicked(String args)
	{
		...
		// default values
		param.svmType = Parameters.C_SVC;
		param.kernelType = Parameters.RBF;
		param.degree = 3;
		param.gamma = 0;
		param.coef0 = 0;
		param.nu = 0.5;
		...
		for(int i=0;i<argv.length;i++)
		{
			...
			switch(argv[i-1].charAt(1))
			{
				...
				case 'w':
					...
					param.weightLabel[param.nrWeight-1] = atoi(argv[i-1].substring(2));
					param.weight[param.nrWeight-1] = atof(argv[i]);
					break;
				default:
					System.err.print("unknown option\n");
			}
		...
		else if(param.svmType == Parameters.EPSILON_SVR ||
			param.svmType == Parameters.NU_SVR)
		{
			...
			for(int i=1;i<XLEN;i++)
			{
				...
				if(param.svmType == Parameters.EPSILON_SVR)
				{
		...
		else
		{
			...
			for (int i = 0; i < XLEN; i++)
				for (int j = 0; j < YLEN ; j++) {
					...
					if (param.svmType == Parameters.ONE_CLASS && d<0) d=2;
	...
	void button_save_clicked(String args)
	{
		...
		try {
			...
			int n = point_list.size();
			if(svm_type == Parameters.EPSILON_SVR || svm_type == Parameters.NU_SVR)
			{
				for(int i=0;i<n;i++)
				{
					point p = point_list.elementAt(i);
